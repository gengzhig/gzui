...
const dragstart = (e, comp) => {
	...
	// drop 松手时，根据拖拽组件添加进去
	ctx.parent.ctx.canvasRef.addEventListener("dragenter", dragenter);
	ctx.parent.ctx.canvasRef.addEventListener("dragover", dragover);
	ctx.parent.ctx.canvasRef.addEventListener("dragleave", dragleave);
	ctx.parent.ctx.canvasRef.addEventListener("drop", drop);
	currentComp = comp;
};
...
const dragend = e => {
	if (ctx.parent.ctx.canvasRef.removeListener) {
		ctx.parent.ctx.canvasRef.removeListener("dragenter", dragenter);
		ctx.parent.ctx.canvasRef.removeListener("dragover", dragover);
		ctx.parent.ctx.canvasRef.removeListener("dragleave", dragleave);
		ctx.parent.ctx.canvasRef.removeListener("drop", drop);
	}
};
