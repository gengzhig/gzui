		...
		<div v-loading="config.loading" class="table-wrapper">
				...
				<div v-for="(tItem, tIndex) in config.tableHeaders" :key="tIndex">
						...
						<template slot-scope="props">
							<el-form label-position="right" class="demo-table-expand table-expand-form">
								<template v-for="(gbItem, gbIndex) in config.tableHeaders">
									...
									>
										<span
											v-if="gbItem.formatter"
											v-html="gbItem.formatter(props.row[gbItem.prop], props.row, props.$index)"
										>
										</span>
										<span v-if="!gbItem.formatter" :title="props.row[gbItem.prop]">
											{{
												props.row[gbItem.prop] !== "" &&
												props.row[gbItem.prop] !== undefined &&
												props.row[gbItem.prop] !== null
													? props.row[gbItem.prop]
													: "-"
											}}
										</span>
					...
					>
						<template slot-scope="scope">
							...
							<div v-else>
								<span v-if="tItem.formatter" v-html="tItem.formatter(scope.row[tItem.prop], scope.row, scope.$index)">
								</span>
								<span v-if="!tItem.formatter" :title="scope.row[tItem.prop]" class="table-content">{{
									scope.row[tItem.prop] !== "" && scope.row[tItem.prop] !== undefined && scope.row[tItem.prop] !== null
			...
			<div class="pagingBox">
				<div v-if="config.total > 0" class="bathBtnBox">
					...
					<el-button
						:type="(batchOperate.base && batchOperate.base.type) || 'primary'"
						v-if="batchOperate && batchOperate.base"
						@click="batchInfo"
					>
					...
					<el-button
						type="primary"
						v-if="batchOperate && batchOperate.export"
						@click="batchExport(batchOperate.export)"
					>
...
export default {
	...
	methods: {
		...
		formatterSummaryData(headers, sumData) {
			let sum = [];
			// 过滤掉isShow为false的列
			let filterHeaders = headers.filter(
				h => h.isShow || h.type == "index" || h.type == "expand" || h.type == "checkbox"
			);
		...
		indexMethod(index) {
			return this.config.listQuery
				? (this.config.listQuery.page - 1) * this.config.listQuery.limit + index + 1
				: index + 1;
		},
		// 表格复选框默认勾选
